...
void *tfunc2(void *argp) { //function that calls LQPUT
	...
	sleep(2);	//second thread waits 1 seconds before attempting PUT

	lhput(lhp, (void*)cp3, cp3->plate, strlen(cp3->plate));
...
int main (void) {
	...
	lhashtable_t *lhp; //ONE agnostic queue that both threads try to alter
	lhp = lhopen(10); //returns pointer to newly created queue_t

	setHashDelay(4);
	
	if(pthread_create(&tid1,NULL,tfunc1,lhp) != 0 ) {
		exit(EXIT_FAILURE);
	}

	if(pthread_create(&tid2,NULL,tfunc2,lhp) != 0 ) {
		exit(EXIT_FAILURE);
	}
